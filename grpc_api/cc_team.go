...
func (api *agentTeam) SearchAgentTeam(ctx context.Context, in *engine.SearchAgentTeamRequest) (*engine.ListAgentTeam, error) {
	...
	if permission.Rbac {
		list, endList, err = api.app.GetAgentTeamsPageByGroups(session.Domain(in.DomainId), session.RoleIds, req)
	} else {
...
func (api *agentTeam) ReadAgentTeam(ctx context.Context, in *engine.ReadAgentTeamRequest) (*engine.AgentTeam, error) {
	...
	if permission.Rbac {
		var perm bool
		if perm, err = api.app.AgentTeamCheckAccess(session.Domain(in.GetDomainId()), in.GetId(), session.RoleIds, auth_manager.PERMISSION_ACCESS_READ); err != nil {
			return nil, err
		} else if !perm {
...
func (api *agentTeam) UpdateAgentTeam(ctx context.Context, in *engine.UpdateAgentTeamRequest) (*engine.AgentTeam, error) {
	...
	if permission.Rbac {
		var perm bool
		if perm, err = api.app.AgentTeamCheckAccess(session.Domain(in.GetDomainId()), in.GetId(), session.RoleIds, auth_manager.PERMISSION_ACCESS_UPDATE); err != nil {
			return nil, err
		} else if !perm {
...
func (api *agentTeam) DeleteAgentTeam(ctx context.Context, in *engine.DeleteAgentTeamRequest) (*engine.AgentTeam, error) {
	...
	if permission.Rbac {
		var perm bool
		if perm, err = api.app.AgentTeamCheckAccess(session.Domain(in.GetDomainId()), in.GetId(), session.RoleIds, auth_manager.PERMISSION_ACCESS_DELETE); err != nil {
			return nil, err
		} else if !perm {
