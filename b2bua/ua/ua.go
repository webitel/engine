...
import (
	...
	"github.com/webitel/engine/b2bua/auth"
...
func NewUserAgent(config *UserAgentConfig) *UserAgent {
	...
	s := config.SipStack
	_ = s.OnRequest(sip.INVITE, ua.handleInvite)
	_ = s.OnRequest(sip.ACK, ua.handleACK)
	_ = s.OnRequest(sip.BYE, ua.handleBye)
	_ = s.OnRequest(sip.CANCEL, ua.handleCancel)
	_ = s.OnRequest(sip.UPDATE, ua.handleUpdate)
	return ua
}
...
func (ua *UserAgent) BuildRequest(
	...
	return ua.buildRequest(method, from, to, contact, recipient, routes, callID)
}
...
func (ua *UserAgent) Recovery(profile *account.Profile, recipient sip.SipUri, sess *session.Session, sdp *string) error {
	from := sess.LocalURI()
	to := sess.RemoteURI()
	contact := profile.Contact()
	request, err := ua.BuildRequest(sip.INVITE, &from, &to, contact, recipient, profile.Routes, sess.CallID())
	if err != nil {
		return err
	}
	...
	resp, err := ua.RequestWithContext(context.Background(), *request, authorizer, false, 1)
	if err != nil {
		return err
	}
	...
	if resp != nil {
		stateCode := resp.StatusCode()
		return fmt.Errorf("Invite recovery session is unsuccessful, code: %d, reason: %s", stateCode, resp.String())
	}
	...
	if ok {
		...
		if toHdr, ok := request.To(); ok && toHdr.Params.Has("tag") {
			...
			if found {
				...
				is := session.NewInviteSession(ua.RequestWithContext, "UAS", contactHdr, request, *callID, transaction, session.Incoming)
	...
	if request.IsInvite() {
		if callID, ok := request.CallID(); ok {
			...
			if _, found := ua.iss.Load(NewSessionKey(*callID, branchID)); !found {
				...
				contactAddr := ua.updateContact2UAAddr(request.Transport(), contactHdr.Address)
				contactHdr.Address = contactAddr
				is := session.NewInviteSession(ua.RequestWithContext, "UAC", contactHdr, request, *callID, cts, session.Outgoing)
				ua.iss.Store(NewSessionKey(*callID, branchID), is)
