...
func (c *Controller) Screenshot(ctx context.Context, session *auth_manager.Session, toUserId int64, fromSockId string) model.AppError {
	...
	return c.app.Screenshot(ctx, session.Domain(0), toUserId, fromSockId)
}

func (c *Controller) RequestScreenShare(ctx context.Context, session *auth_manager.Session, fromUserId, toUserId int64, sockId string, sdp, id string) (string, model.AppError) {
	if !session.HasAction(model.PermissionControlAgentScreen) {
		return "", c.app.MakeActionPermissionError(session, model.PermissionControlAgentScreen, auth_manager.PERMISSION_ACCESS_READ)
	}
...
func (c *Controller) AcceptScreenShare(_ context.Context, session *auth_manager.Session, toUserId int64, sockId string, sess, sdp string, fromSockId string) model.AppError {

	return c.app.AcceptScreenShare(session.Domain(0), toUserId, sockId, sess, sdp, fromSockId)
}
