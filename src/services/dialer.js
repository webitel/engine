...
let Service = {
    ...
    listHistory: function (caller, option, cb) {
        checkPermissions(caller, 'dialer', 'r', (err) => {
            ...
            if (typeof option.filter !== "object" || !option.filter)
                option.filter = {};
            ...
            dbDialer.memberById(options.member, options.dialer, (err, memberDb, dialerDb) => {
                ...
                dbDialer._updateMember(
                    ...
                    (e, r) => {
                        ...
                        application.PG.getQuery('agents').setReadyNow(options.dialer, options.member, e => {
                            if (e) {
                                log.error(`set ready agent error: ${e.message}`);
                            } else {
                                log.trace(`set ready agent success`);
                            }
                        })
