...
from  r
	...
	if err != nil {
		return nil, model.NewCustomCodeError("store.sql_region.create.app_error", fmt.Sprintf("name=%v, %v", region.Name, err.Error()), extractCodeFromErr(err))
	}
...
func (s SqlRegionStore) GetAllPage(ctx context.Context, domainId int64, search *model.SearchRegion) ([]*model.Region, model.AppError) {
	...
	err := s.ListQueryFromSchema(ctx, &region, "flow", search.ListRequest,
		...
		model.Region{}, f)
	if err != nil {
		return nil, model.NewCustomCodeError("store.sql_region.get_all.app_error", err.Error(), extractCodeFromErr(err))
	}
...
where r.domain_id = :DomainId and r.id = :Id`, map[string]interface{}{
	...
	if err != nil {
		return nil, model.NewCustomCodeError("store.sql_region.get.app_error", err.Error(), extractCodeFromErr(err))
	}
...
from r
	...
	if err != nil {
		return nil, model.NewCustomCodeError("store.sql_region.update.app_error", err.Error(), extractCodeFromErr(err))
	}
...
func (s SqlRegionStore) Delete(ctx context.Context, domainId int64, id int64) model.AppError {
	if _, err := s.GetMaster().WithContext(ctx).Exec(`delete from flow.region c where c.id=:Id and c.domain_id = :DomainId`,
		...
		return model.NewCustomCodeError("store.sql_region.delete.app_error", fmt.Sprintf("Id=%v, %s", id, err.Error()), extractCodeFromErr(err))
	}
	return nil
}
