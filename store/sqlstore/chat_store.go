...
from (
     select 1 pri, inv.created_at, null::uuid id, inv.id::uuid invite_id, null::timestamptz joined_at, inv.conversation_id, inv.user_id, inv.created_at updated_at, inv.props, null::timestamptz as closed_at
     from chat.invite inv
     where inv.user_id = :UserId::int8 and inv.closed_at isnull
        and inv.domain_id = :DomainId::int8
     union

     select 2, ch.created_at, ch.id::uuid, null::uuid invite_id, ch.created_at as joined_at, ch.conversation_id, ch.user_id, ch.updated_at, ch.props, ch.closed_at
     from (
		...
		where ch.user_id = :UserId::int8
		  and (ch.closed_at isnull or exists(select 1
											 ...
											 where a.user_id = :UserId::int8 and mat.agent_call_id = ch.id::text and mat.state != 'leaving'))
