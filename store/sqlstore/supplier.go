...
import (
	...
	driversql "github.com/webitel/webitel-go-kit/storage/sql"
...
func NewSqlSupplier(settings model.SqlSettings) *SqlSupplier {
	...
	supplier.oldStores.schemeVariable = NewSqlSqlSchemeVariablesStore(supplier)
...
func setupConnection(con_type string, dataSource string, settings *model.SqlSettings) *gorp.DbMap {
	dn, err := driversql.Register(*settings.DriverName, driversql.TraceAll(), driversql.WithTracer(otelsql.NewTracer(otelsql.TraceQueryWithArgs())))
	if err != nil {
...
func (ss *SqlSupplier) SchemeVariable() store.SchemeVariablesStore {
	return ss.oldStores.schemeVariable
}
