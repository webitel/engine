...
func (a *App) GetAgentStatusStatistic(domainId, supervisorId int64, search *model.SearchAgentStatusStatistic) ([]*model.AgentStatusStatistics, bool, *model.AppError) {
	list, err := a.Store.Agent().StatusStatistic(domainId, supervisorId, search)
	if err != nil {
		return nil, false, err
	}
	search.RemoveLastElemIfNeed(&list)
